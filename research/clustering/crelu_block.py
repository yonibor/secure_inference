...
class ClusterRelu(nn.Module):
    def __init__(
        ...
        self.channel_indices = nn.Parameter(
            torch.arange(C).view(C, 1, 1).repeat(1, H, W), requires_grad=False
        )  # technical, used for selecting the prototypes

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        not_active_channels = [
            c for c in range(x.shape[1]) if c not in self.active_channels
        ]
        ...
        x_inter = (
            x[:, self.active_channels] * (1 - active_inter) + prototype_x * active_inter
        )
    ...
    def inter(self, new_inter: Optional[Union[np.ndarray, float, torch.Tensor]]):
        if new_inter is None:
            ...
            if isinstance(new_inter, np.ndarray):
                ...
                new_inter = torch.full(
                    (self.C, self.H, self.W), new_inter, dtype=torch.float
                )
            assert torch.all(
                torch.logical_and(new_inter >= 0, new_inter <= 1)
    ...
    def active_channels(
        self, new_channels: Union[torch.Tensor, np.ndarray, list]
    ) -> None:
