...
from research.distortion.utils import get_channels_subset, get_model
...
class DistortionUtils:
    def __init__(self, gpu_id, params, checkpoint, cfg, mode, seed=123):
        ...
        self.model = get_model(
            config=self.cfg, gpu_id=self.gpu_id, checkpoint_path=self.checkpoint
        )
    ...
    def get_loss(self, out, ground_truth):
        if self.cfg.model.type == "ImageClassifier":
            return (
                self.model.head.loss(out, ground_truth.to(torch.long))["loss"]
                .cpu()
                .numpy()
            )
        elif self.cfg.model.type == "EncoderDecoder":
            return (
                self.model.decode_head.losses(out, ground_truth)["loss_ce"]
                .cpu()
                .numpy()
            )
        else:
    ...
    def get_samples(self, batch_index, batch_size):
        ...
        if self.cfg.model.type == "ImageClassifier":
            batch = torch.stack(
                [self.dataset[sample_id]["img"].data for sample_id in batch_indices]
            ).to(self.device)
            ground_truth = torch.Tensor(
                [self.dataset[sample_id]["gt_label"] for sample_id in batch_indices]
            ).to(self.device)
        elif self.cfg.model.type == "EncoderDecoder":
            batch = torch.stack(
                [self.dataset[sample_id]["img"][0].data for sample_id in batch_indices]
            ).to(self.device)
            ground_truth = torch.stack(
                [
                    self.dataset[sample_id]["gt_semantic_seg"][0].to(torch.int64)
    ...
    @staticmethod
    def get_distortion(
        block_name_to_activation_baseline, block_name_to_activation_distorted
    ):
    ...
    def get_activations(
        ...
        with torch.no_grad():
            with model_block_relu_transform(
                self.model, block_size_spec, self.arch_utils
            ) as model:
                torch.cuda.empty_cache()
                input_block_index = np.argwhere(
                    np.array(self.params.BLOCK_NAMES) == input_block_name
                )[0, 0]

                output_block_indices = [
                    np.argwhere(np.array(self.params.BLOCK_NAMES) == output_block_name)[
                        0, 0
                    ]
                    for output_block_name in output_block_names
                ]
                ...
                for block_index in range(
                    input_block_index, max(output_block_indices) + 1
                ):
                    block_name = self.params.BLOCK_NAMES[block_index]
                    activation = self.arch_utils.run_model_block(
                        model, activation, self.params.BLOCK_NAMES[block_index]
                    )
    ...
    def get_batch_data(self, batch_index, batch_size, block_size_spec_id):
        ...
        input_images, ground_truth = self.get_samples(
            batch_index, batch_size=batch_size
        )

        block_name_to_activation, losses = self.get_activations(
            block_size_spec,
    ...
    def get_batch_distortion(
        ...
        block_name_to_activation_baseline, _, losses_baseline = self.get_batch_data(
            batch_index, batch_size, id(baseline_block_size_spec)
        )

        input_tensor = block_name_to_activation_baseline[
            self.params.BLOCK_INPUT_DICT[input_block_name]
        ]

        block_name_to_activation_distorted, losses_distorted = self.get_activations(
            block_size_spec,
...
class ChannelDistortionHandler:
    def __init__(
        ...
        config,
        is_train_mode=False,
        baseline_block_size_spec_path=None,
        ...
        self.distortion_utils = DistortionUtils(
            gpu_id=self.gpu_id,
            params=self.params,
        ...
        if self.baseline_block_size_spec_path:
            ...
            self.baseline_block_size_spec = pickle.load(
                open(self.baseline_block_size_spec_path, "rb")
            )
        else:
        ...
        if self.clean_block_size_spec_path:
            ...
            self.clean_block_size_spec = pickle.load(
                open(self.clean_block_size_spec_path, "rb")
            )
        else:
            self.clean_block_size_spec = dict()

    def extract_deformation_channel_ord(self, batch_index: int, batch_size: int):
        ...
        for layer_name in self.params.LAYER_NAMES:
            ...
            if layer_name not in block_size_spec:
                block_size_spec[layer_name] = np.ones(
                    shape=(layer_num_channels, 2), dtype=np.int32
                )

            for channel in tqdm(
                channels_to_run[layer_name],
                desc=f"Batch={batch_index} Layer={layer_name}",
            ):

                file_name = os.path.join(
                    self.output_path, f"{layer_name}_{channel}_{batch_index}.npy"
                )
...
if __name__ == "__main__":
    ...
    chd.extract_deformation_channel_ord(
        batch_index=args.batch_index, batch_size=args.batch_size
    )
